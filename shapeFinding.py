...
def mode_select():
    ...
    # // 16 , then compute each entropy
    img_i = new_represent(img) // 32
    img_ii = new_represent2(img) // 32

    sta_i = np.zeros((32,))
    # 创建一个初始化32维用0填充的数组[0,0,...,0]
    sta_ii = np.zeros((32,))
    ...
    print('entropy_i:',entropy_i,'entropy_ii:', entropy_ii)
    if entropy_i > entropy_ii:
        print('mode ii : 8, 5, (1, 1), (1, 2)')
        return 8, 5, (1, 1), (1, 2), 'ii'
    else:
        print('mode i : 4, 4, (1, 2), (1, 2)')
        return 4, 4, (1, 2), (1, 2), 'i'
        
    # 分别是 layerstart_y,layerstart_uv,rough_y,rough_uv
## 看熵编码哪一个编码更少，选取哪个,ie. entropy更小的那个prediction mode
...
class DetailSearch:
    ...
    def main(self):
        ...
        for f in img_path_total:
            ...
            print('\rdetail Searching for image %d' % search_num, end='')
        ...
        print('\ndetail Search is complete，it has been saved as %s' % output_name)
...
class RoughSearch:
    ...
    def main(self):
        ...
        for f in img_path_total:
            ...
            print('\rrough Searching for image %d' % search_num, end='')
            search_num = search_num + 1
        ...
        print('\nrough Search is complete，it has been saved as %s' % output_name)
...
class ShapeSearch:
    ...
    def main(self):
        ...
        print('\nshape Search is complete,it has been saved as %s' % output_name)
...
def ShapeFinding(dataset):
    # input_dir and output_dir has been defined in fun main() in each class
    output_dir = 'frequency'  # output folder
    # layer interface
    layer_shape = 4
    # define is initial num, will be changed later with mode, so concentrate on the mode return
    batch_size = 1  # batch size
    shape_degree = 0.5  # delete degree
    shape_height, shape_width = ([1, 4], [1, 4])  # shape height and width
    imgProcessing.dir_check(output_dir, empty_flag=True)  # empty the output folder
    # Mode
    if dataset == 'Massachusetts':
        ...
        layer_shape_y, layer_shape_uv, rough_y, rough_uv, mode =  4, 4, (1,2), (1,2), 'i'
        print(mode)
    ...
    for component in component_total:
        ...
        TS.main()
        print('component',component,'has been finished')
    return mode

if __name__ == '__main__':
    print('start shapefinding')
    dataset = 'mnist'
    mode = ShapeFinding(dataset)
    print(mode)
    print('ending')

    # mode i : new represent simple, 4,4
    # mode ii : new represent2 complex 5,8