...
def decode(tt):
    ...
    height = int(tt[8:8 + bit_height], 2)  # Get img height
    width = int(tt[8 + bit_height:8 + bit_height + bit_width], 2)  # Get img width
    # 新的tt
...
def decode_capacity(tt, height, width, decode_rough, decode_detail, decode_shape, bit_height,
    ...
    tt = tt[10:]  
    # Delete the high position value and the wide position value
    if len(tt) != 0 and Golomb_m != 1:
        first_height = int(tt[0:bit_height],
                           2)  
        # Determine the height in the starting cell and convert it to an integer type
        first_width = int(tt[bit_height:bit_height + bit_width],
                          2) 
        # Determine the width in the starting cell and convert it to an integer type
        ...
        # 定位到第一个shape结束的像素点坐标，第(last location)th 个像素
...
def Decode():
    ...
    codebook_dir = 'codebook'  # codebook                         
    print('\n', "%" * 150)
    imgProcessing.dir_check(output_dir, empty_flag=True)  # empty the output dir
